...
If you discover a security vulnerability in biralo, please report it by:
...
chmod 600 ~/.biralo/config.json

# ❌ Bad: Hardcoding keys in code or committing them
```

**Recommendations:**
- Store API keys in `~/.biralo/config.json` with file permissions set to `0600`
...
- ✅ Run biralo with a dedicated user account
...
- Keep authentication data in `~/.biralo/whatsapp-auth` secure (mode 0700)
...
- Subscribe to security advisories for biralo and its dependencies
...
3. **Set Proper Permissions**
   ...
   chmod 700 ~/.biralo
   chmod 600 ~/.biralo/config.json
   chmod 700 ~/.biralo/whatsapp-auth
   ```

4. **Enable Logging**
   ```bash
   # Configure log monitoring
   tail -f ~/.biralo/logs/biralo.log
   ```
...
- **Chat history is stored locally** - protect the `~/.biralo` directory
...
2. **Review logs for unauthorized access**
   ```bash
   grep "Access denied" ~/.biralo/logs/biralo.log
   ```
...
- GitHub Security Advisories: https://github.com/HKUDS/biralo/security/advisories
- Release Notes: https://github.com/HKUDS/biralo/releases
